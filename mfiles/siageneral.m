...
for n=1:N
  ...
  Dup  = 0.5 * ( Gamma(j,nk)+ Gamma(j,k)  ) .* Hup.^5 .* a2up;
  Ddn  = 0.5 * ( Gamma(j,k) + Gamma(j,sk) ) .* Hdn.^5 .* a2dn;
  Drt  = 0.5 * ( Gamma(ej,k)+ Gamma(j,k)  ) .* Hrt.^5 .* a2rt;
  Dlt  = 0.5 * ( Gamma(j,k) + Gamma(wj,k) ) .* Hlt.^5 .* a2lt;
  
  % call *adaptive* diffusion() to time step H
  [H] = newdiffusion(Lx,Ly,J,K,Dup,Ddn,Drt,Dlt,H,deltat,M,b);
